...
function ModalEditTech({title, tech, id ,showModalEdit, setShoModalEdit}) {
    ...
    function editTechInfo({status}){
        const statusOBj = {status}
        api.put(`/users/techs/${id}`, statusOBj,{
            headers:{
    ...
    function deleteTech(){
        /* const value = tech.id.splice(0,1) */
            api.delete(`/users/techs/ `,{
                headers:{
                    Authorization: `Bearer ${token}`
                }
    ...
    return (
        <S.Container showModalEdit={showModalEdit}>
            <S.Header>
                <p>Tecnologia Detalhes</p>
                <svg onClick={()=>setShoModalEdit("")} width="11" height="13" viewBox="0 0 11 13" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <S.Body>
                <form onSubmit={handleSubmit(editTechInfo)}>
                    ...
                    <input placeholder={tech.title} {...register("title")}/>
                    ...
                    <div>
                        <Button type="submit" color='color-primary-Disable'>Salvar alterações</Button>
                        <Button type= "button" onClick={deleteTech} color='grey1'>Excluir</Button>
